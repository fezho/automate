...
func NewPostgresService(l logger.Logger, connFactory *secureconn.Factory, migrationsConfig migration.Config,
	...
	// TODO (tc): Can remove?
	resp, err := authzPoliciesClient.GetPolicyVersion(context.Background(), &authz.GetPolicyVersionReq{})
	if err != nil {
		return nil, err
	}

	p, err := postgres.New(l, migrationsConfig, resp.Version.Major == authz.Version_V2, authzAuthorizationClient)
	if err != nil {
		return nil, err
	}
